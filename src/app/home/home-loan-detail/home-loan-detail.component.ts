...
@Component({
  ...
  styleUrls: ['./home-loan-detail.component.css']
})
export class HomeLoanDetailComponent implements OnInit {
  ...
  httpGet() {
    this.loading = true;
    this.http.get<any>(environment.api + "loan/detail/"+this.id, { headers: this.configService.headers() })
    .subscribe(
      data=>{
        this.loading = false;
  ...
  onSubmit() {
    ...
    this.http.post<any>(environment.api + "loan/update", body, {
